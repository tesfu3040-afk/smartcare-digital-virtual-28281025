    ...
    <div>
      <section className="relative py-20 overflow-hidden" style={{ background: 'linear-gradient(135deg, hsl(211, 80%, 35%) 0%, hsl(211, 80%, 42%) 50%, hsl(199, 89%, 40%) 100%)' }}>
        ...
        <div className="container text-center max-w-3xl relative z-10">
          <h1 className="font-display text-4xl md:text-5xl font-extrabold text-white">{t("doctors.heroTitle")}</h1>
          <p className="mt-4 text-lg text-white/80">{t("doctors.heroDesc")}</p>
        </div>
      </section>
      ...
      <section className="container py-10">
          ...
          <Select value={specialty} onValueChange={setSpecialty}>
            ...
            <SelectContent>
              <ScrollArea className="max-h-60">
                <SelectItem value="All Specialties">{t("doctors.allSpecialties")}</SelectItem>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-muted-foreground">
            <p className="text-lg">{t("doctors.noResults")}</p>
            <p className="text-sm mt-2">{t("doctors.adjustSearch")}</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((d, i) => (
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <Badge variant="secondary" className="mt-1">{d.specialty || t("doctors.general")}</Badge>
                      </div>
                    </div>
                    <p className="text-sm text-muted-foreground line-clamp-2 mb-4">
                      {d.bio || t("doctors.defaultBio")}
                    </p>
                    <div className="grid grid-cols-3 gap-2 text-xs text-muted-foreground mb-4">
                      <div className="flex items-center gap-1">
                        <Clock className="h-3.5 w-3.5" /> {d.experience_years} {t("doctors.yrExp")}
                      </div>
                      <div className="flex items-center gap-1">
                    ...
                    <Button className="w-full" size="sm" asChild>
                      <Link to={`/book/${d.user_id}`}>
                        <Calendar className="h-4 w-4 mr-1" /> {t("doctors.book")}
                      </Link>
                    </Button>
                  </CardContent>
                </Card>
